...
for (k in folds){
  ...
  data_train=subset(data,!(Subject %in% k))
  #Create test dataset, data in fold k
  data_test=subset(data,Subject %in% k)
  
  #------ train model - apply model to data_train ------
  logis_model = glm(Diagnosis ~ range, family = binomial, data_train)
  predict_train = predict(logis_model, data_train, type = 'response')
  predict_train = ifelse(data$predicted > 0.5, 1, 0)
  ...
  predict_test=predict(logis_model, data_test, type = 'response')
  predict_test = ifelse(data$predicted > 0.5, 1, 0)
  cm_train = confusionMatrix(data = predict_test, reference = data_test$Diagnosis)
  
  #------ save the performance ------ 
  #Fills out the kth row in cv_empty with iteration and rmse's
  #cv_empty[n,]=cbind(n, cm_train$overall[1], cm_test$overall[1])
  print(cm_test$overall[1])
  n=n+1
  }

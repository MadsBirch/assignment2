...
ggplot(d)+geom_density(aes(mean))

ggplot(d, aes(mean))+ 
  geom_histogram(aes(y=..density..))+ 
  stat_function(fun=dnorm, args=list(mean(d$mean),sd(d$mean)))

ggplot(d, aes(Diagnosis, mean, fill = Diagnosis))+  geom_bar(stat="summary",fun.y=mean)+geom_errorbar(stat="summary",fun.data=mean_cl_boot, width = 0.2)
...
full_df = left_join(d, DemoData,  by = "Subject", "Diagnosis", "Study")

full_df = merge(d, DemoData, by.y = "Subject", "Diagnosis", all.x = T)

# 
# DemoData = DemoData$variable = rownames(DemoData)
# 
# full_df = merge(d, DemoData, by.x = 2, by.y = 0, all.x = TRUE)
# 
# merge(dat1, dat2)
# 
# full_df = cbind(d, DemoData)
